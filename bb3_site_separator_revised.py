...
def get_next_line(line_of_text):
    line_fmt = (
        '{:2d}/{:2d}/{:2d}	{:2d}:{:2d}:{:2d}	'
    ...
    parsed_line = parse(line_fmt, line_of_text)
        ...
        return time_of_sample, dt_current, dt_prev
...
# lines = f.readlines()
# for line_n in len():
#     this_line = lines[line_n]
#     next_line = lines[line_n+1]
#     prev_line = lines[line_n-1]
while line_of_text:
        
        try:
            time_of_sample, dt_current, dt_prev = get_next_line(line_of_text)
        except Exception:
        ...
        if dt_current - dt_prev < timedelta(milliseconds=0):
            ...
            # TODO: check if the next timedelta(s) are < 0
            u_inp = input('do you want to start a new file?')
            if u_inp == 'y':
                dt_prev = dt_current
                i = dt_current.strftime("%d_%m_%yT%H%M%S")
                # this is so can look up timestamp on spreadsheet and label site
